...
AllSpikingtimeRange ={[0.25 0.75], [0.25 0.75]};% Spike Analysis Period
spikeElecCutOffs_AllMonkeys =  {[4000 1.2 1 1],[4000 1.2 1 1],[];[10000 1.8 6 1],[10000 1.8 6 1],[]};% TotalSpikesInTheSession SNR Stimulus Firing Rate  TransientFlag
...
if runHighRmsLFPFlag == 1
    UsegoodSpikingElecForLFPFlag = 0; % 1 - Do LFP analysis on only those electrodes which had good firing rate also
...
for idelta = 1:2 % relative orientation difference between the two gratings (0 or 90)
    ...
    for iMon =1:length(Indices_parallel)
        ...
        if runGoodSpikeElecSpikeFlag == 1
            ...
            for ilenSpikeCutOffs = 1:size(spikeElecCutOffs_AllMonkeys,1)
                
                SpikingtimeRange = AllSpikingtimeRange{ilenSpikeCutOffs};
                spikeElecCutOffs = spikeElecCutOffs_AllMonkeys{ilenSpikeCutOffs,iMon};
                
                if iMon~=size(spikeElecCutOffs_AllMonkeys,2)
                    [SpikeResults{ilenSpikeCutOffs,idelta,iMon}] = getDualTF_SmallStim_SpikeVals(indicesToUse,folderSourceString,gridType, ...
        ...
        if runHighRmsLFPFlag== 1
            [Results{idelta,iMon}] = getDualTF_SmallStim_LFPVals_sessionWise(indicesToUse,folderSourceString,gridType, ...
                ...
                UsegoodSpikingElecForLFPFlag,spikeElecCutOffs,SpikingtimeRange,...
